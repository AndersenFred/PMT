...
def get_tot_ampl(y_data, Measurement_time, threshold = 0):
...
def ampl_hist(ampl, folder, SN):
    if not os.path.isdir(f"{folder}/ToT_{SN}"):
        os.mkdir(f"{folder}/ToT_{SN}")
    hi, bins = np.histogram(ampl, bins=40, range=(0, 1.5))
    hi[0] = 0
    bins = bins[:-1]
    maximum = 8 + np.argmax(hi[8:])
    plus_max = np.min([8, len(bins) - maximum-1])
    ...
    plus_max = int(np.min([int(p_opt[2] / bins[1]), len(bins) - int(p_opt[1]/bins[1]) - 1]))

    maximum = int(p_opt[1] / bins[1])
    p_opt, cov = opt.curve_fit(normal, bins[maximum - int(plus_max / bins[1]):maximum + int(plus_max / bins[1])],
                               hi[maximum - int(plus_max / bins[1]):maximum + int(plus_max / bins[1])],
                               p0=p_opt, maxfev=50000)
    print(
        f'Mean Amplitude: {p_opt[1]:.2f} pm {np.sqrt(cov[1, 1]):.2f} with sigma: {p_opt[2]:.2f} pm {np.sqrt(cov[2, 2]):.2f},')
    fig, ax = plt.subplots()
    ax.bar(bins, hi, width=bins[1], label='Data')
    ax.axvline(bins[int(p_opt[1] / bins[1])], color="r", label='Maximum')
    #ax.axvspan(bins[int((p_opt[1] - plus_max) / bins[1]) + 1], bins[int((p_opt[1] + plus_max) / bins[1])], facecolor="C1", alpha=0.35)
    ax.plot(bins[int((p_opt[1]/ bins[1] - plus_max) ) + 1: 1 + int((p_opt[1]/ bins[1]) + plus_max) ],
            normal(bins[int((p_opt[1]/ bins[1]) - plus_max) + 1: 1 + int((p_opt[1]/ bins[1]) + plus_max) ], *p_opt), 'm', label='Gaussian fit')
    plt.xlabel("Amplitude in V")
    plt.ylabel("count")
    plt.title(f"Voltage for {SN}")
    plt.grid()
    plt.tight_layout()
    fig.savefig(f"{folder}/ToT_{SN}/{SN}_ampl_hist.pdf")
    plt.clf()
    plt.close()
    return p_opt, cov

def tot_hist(y_data, tot, folder, SN):
    if not os.path.isdir(f"{folder}/ToT_{SN}"):
        os.mkdir(f"{folder}/ToT_{SN}")
    tot = tot[tot != 0] * 1e9
    print(f'NPHE: { len(tot)/np.shape(y_data)[1]:.3f}')
    hi, bins = np.histogram(tot, bins=71, range=(0, 56))
    bins = bins[:-1]
    maximum = 5 + np.argmax(hi[5:])
    plus_max = np.min([14, len(bins) - maximum-1])
    # noinspection PyTupleAssignmentBalance
    p_opt, cov = opt.curve_fit(normal, bins[maximum - plus_max:maximum + plus_max],
                               hi[maximum - plus_max:maximum + plus_max],
                               p0=[10000, 25, 50], maxfev=50000)
    plus_max = int(np.min([2 * int(p_opt[2] / bins[1]), len(bins) - int(p_opt[1] / bins[1]) - 1]))
    maximum = int(p_opt[1] / bins[1])
    # noinspection PyTupleAssignmentBalance
    p_opt, cov = opt.curve_fit(normal, bins[maximum - plus_max:maximum + plus_max],
                               hi[maximum - plus_max:maximum + plus_max],
    ...
    return p_opt, cov, len(tot)/np.shape(y_data)[1]


def corr_plot(folder, SN, tot, ampl):
    if not os.path.isdir(f"{folder}/ToT_{SN}"):
        os.mkdir(f"{folder}/ToT_{SN}")
    tot = tot*1e9
    fig, ax = plt.subplots()
    ...
    plt.plot(xedges, log_vert(xedges, *p_opt), label="Logarithmic Fit")
    plt.plot(xedges[mask], y, label='Points used for Fit')
    bor = np.nan_to_num(log_vert(xedges, p_opt[0]+np.sqrt(cov[0,0]), p_opt[1]+np.sqrt(cov[1,1]), p_opt[2]+np.sqrt(cov[2,2])))
    ...
    return p_opt, cov, np.sum(pdp) / np.sum(H)


def log_tot(folder, SN, p_opt_hist, cov_hist, p_opt_ampl, cov_ampl, nphe, p_opt_corr, cov_corr, pdp):
    name = f'{folder}\\log_tot_{SN}.txt'
    f = open(name, 'a')
    text = f'Date: {datetime.now()}\n  nphes: {np.round(nphe, 3)}\n fit results tot hist: {np.round(p_opt_hist, 4)}\n cov: {np.round(cov_hist, 4)}\nfit results ampl hist {np.round(p_opt_ampl, 4)}\n cov: {np.round(cov_ampl, 4)}\nfit results corr: {np.round(p_opt_corr, 4)}\n cov: {np.round(cov_corr, 4)} \n partly delayed pulses {pdp}\n\n\n'
    f.write(text)
    f.close()

def tot_to_tex(folder):
    with open("tot_table.tex", "w") as f:
...
def save_first(y, mes_time, tot, folder, SN, n = 10):
    if not os.path.isdir(f"{folder}/ToT_{SN}"):
        os.mkdir(f"{folder}/ToT_{SN}")
    fig, ax = plt.subplots(nrows=2)
    analog = y[2] - y[3]
    digital = y[1] - y[0]
    x = np.linspace(0, mes_time, np.shape(y)[2])*1e9
    for i in range(len(tot)):
        if n <= 0:
            break
        if not tot[i] == 0:
            n -= 1
            ax[0].plot(x, analog[i])
            ax[1].plot(x, digital[i])
    plt.suptitle(f"Waveforms of {SN}")
    ax[0].set_ylabel('Voltage in V')
    ax[1].set_ylabel('Voltage in V')
    ax[1].set_xlabel('Time in ns')
    ax[0].grid()
    ax[1].grid()
    plt.tight_layout()
    plt.savefig(f"{folder}/ToT_{SN}/waveforms_{SN}.pdf")
    plt.clf()
    plt.close()
